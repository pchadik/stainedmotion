...
const StainedGlassScreensaver = () => {
  ...
  const prevHarmonyRef = useRef(0);
  const harmonyMixRef = useRef(1); // 1 = fully on current harmony, <1 = mid cross-fade
  ...
  const harmonicHueFor = (harmony, cx, cy, w, h) => {
    ...
    const pos = (blend + 1) / 2 * harmony.length;
    const i0 = Math.floor(pos) % harmony.length;
    const i1 = (i0 + 1) % harmony.length;
    const frac = pos - Math.floor(pos);
    const h0 = (rootHueRef.current + harmony[i0] + 360) % 360;
    const h1 = (rootHueRef.current + harmony[i1] + 360) % 360;
    return lerpHue(h0, h1, frac);
  };

  const getHarmonicHue = (cx, cy, w, h) => {
    const toHue = harmonicHueFor(harmonies[harmonyNames[harmonyTypeRef.current]], cx, cy, w, h);
    const mix = harmonyMixRef.current;
    if (mix >= 1) return toHue;
    // Cross-fade from the previous harmony so a switch sweeps in smoothly.
    const fromHue = harmonicHueFor(harmonies[harmonyNames[prevHarmonyRef.current]], cx, cy, w, h);
    return lerpHue(fromHue, toHue, mix);
  };
  ...
  const shiftColors = (w, h) => {
    ...
    if (harmonyMixRef.current < 1) {
      // Advance an in-progress cross-fade (~1.7s at 60fps).
      harmonyMixRef.current = Math.min(1, harmonyMixRef.current + 0.01);
    } else if (Math.random() < 0.00005) { // Rare harmony changes
      prevHarmonyRef.current = harmonyTypeRef.current;
      harmonyTypeRef.current = (harmonyTypeRef.current + 1) % harmonyNames.length;
      harmonyMixRef.current = 0;
    }
  };
