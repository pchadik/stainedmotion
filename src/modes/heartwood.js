...
(function (App) {
  App.createHeartwood = function (deps) {
    ...
    var ROOT_SEGS = 12;
    // One root from a tree's ground point, growing near-horizontally outward and
    // dipping down, wavering over time.
    function growTreeRoot(ctx, gx, gy, side, seed, t, ax, ay, w, h, thick, rhue, rsat, rl) {
      var len = minDim * (0.05 + 0.025 * ((seed * 13) % 5) / 5);
      var step = len / ROOT_SEGS, margin = minDim * 0.01, ph = seed * 1.7;
      var x = gx, y = gy, prev = { x: x, y: y };
      var ang0 = side > 0 ? 0.15 : Math.PI - 0.15;         // near-horizontal, outward
      for (var i = 1; i <= ROOT_SEGS; i++) {
        var prog = i / ROOT_SEGS;
        var ang = ang0 + Math.sin(prog * 4 + ph + t * 0.6) * 0.3 * (1 - 0.3 * prog);
        ang += (Math.PI / 2 - ang) * 0.12 * prog;          // dip downward along the length
        x += Math.cos(ang) * step; y += Math.sin(ang) * step;
        var wd = thick * Math.pow(1 - prog, 0.7) + 0.4;
        var style = 'hsla(' + rhue + ', ' + rsat + '%, ' + rl + '%, ' + (0.6 * (1 - 0.45 * prog)).toFixed(3) + ')';
    ...
    function drawRoots(ctx, w, h, t, ax, ay) {
      var gy = h * 0.83, margin = minDim * 0.01;
      for (var i = 0; i < trees.length; i++) {
        ...
        var thick = 1.0 + 1.6 * tr.z;
        growTreeRoot(ctx, gx, gy, -1, i, t, ax, ay, w, h, thick, rhue, rsat, rl);
        growTreeRoot(ctx, gx, gy, 1, i + 5, t, ax, ay, w, h, thick, rhue, rsat, rl);
      }
    }
