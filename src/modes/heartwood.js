  ...
  App.createHeartwood = function (deps) {
    ...
    function buildCanopy(w, h) {
      ...
      for (var r = 0; r < rows; r++) {
        for (var c = 0; c <= cols; c++) {
          ...
          seeds.push({ x: sx, y: sy, draw: (sy < drawBot && ex * ex + ey * ey > (Rbase + CH) * (Rbase + CH)) });
        }
      }
    ...
    function drawCanopy(ctx, w, h, t, ax, ay) {
      ...
      for (var i = 0; i < crowns.length; i++) {
        ...
        for (var m = 0; m < M; m++) {
          var p = samp[m], f = m / M;
          var lobe = cr.LOBE * (0.55 * Math.sin(7 * 2 * Math.PI * f + cr.seed) + 0.45 * Math.sin(13 * 2 * Math.PI * f + cr.seed2 + phase));
